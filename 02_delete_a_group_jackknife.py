...
BUNDLE_DIR = ROOT / "main" / "radiology" / "bootstrap_state_bundle"
OUT_DIR = ROOT / "main" / "radiology" / "delete_a_group_jackknife"
...
class NeuralCoxModel:
    ...
    def predict(self, X):
        ...
        x = torch.as_tensor(np.asarray(X, dtype=np.float32), device=model_device)
        self.net.eval()
        with torch.no_grad():
...
def weighted_mean(x, w=None):
    ...
    if w is None:
        ...
        if x.size == 0:
            return np.nan, 0.0, 0
        return float(np.mean(x)), float(x.size), int(x.size)

    w = np.asarray(w, dtype=float)
    if x.shape[0] != w.shape[0]:
        raise ValueError(f"x and w must have the same length, got len(x)={x.shape[0]} len(w)={w.shape[0]}")
    ...
    mean = float(np.sum(w_valid * x_valid) / w_sum)
    return mean, w_sum, int(mask.sum())
...
def disparity_table_from_model_counterfactual(
    ...
    if sample_weight is None:
        ...
        if sample_weight.shape[0] != len(X):
            raise ValueError(
                f"sample_weight length={sample_weight.shape[0]} does not match len(X)={len(X)}"
            )
    ...
    for prefix in families:
        ...
        _, weight_sum, n_positive_weight = weighted_mean(lp_ref * 0.0, sample_weight)
        rows.append(
            {
                "group_var": group_var,
                "level": ref_level,
                ...
                "beta": 0.0,
                "HR": 1.0,
                "contrast_weight_sum": weight_sum,
                "contrast_n_positive_weight": n_positive_weight,
            }
        )
        ...
        for fam_col in fam_cols:
            ...
            beta, weight_sum, n_positive_weight = weighted_mean(lp_cf - lp_ref, sample_weight)
...
def build_weighted_point_compare_results(
    ...
    tau_map = {}
    if compare_df is not None and len(compare_df) > 0 and "outcome" in compare_df.columns:
        tau_cols = [col for col in ["tau"] if col in compare_df.columns]
        if tau_cols:
            tau_map = compare_df[["outcome"] + tau_cols].drop_duplicates("outcome").set_index("outcome")["tau"].to_dict()
    ...
    for outcome, by_family in artifact_store.items():
        for family_var, by_scheme in by_family.items():
            ...
            for scheme_name in [left_scheme, right_scheme]:
                ...
                scheme_tables[scheme_name] = disparity_table_from_model_counterfactual(
                    model=artifact["model"],
                    X=artifact["X_model"].reset_index(drop=True).copy(),
                    ref_map=ref_map,
            ...
            cmp = compare_disparity(
                tbl_left=scheme_tables[left_scheme],
                tbl_right=scheme_tables[right_scheme],
                left_scheme=left_scheme,
                right_scheme=right_scheme,
            )
            cmp["outcome"] = outcome
            cmp["family_var"] = family_var
            if outcome in tau_map:
                cmp["tau"] = tau_map[outcome]
            rows.append(cmp)
    ...
    for outcome, by_family in artifact_store.items():
        for family_var, by_scheme in by_family.items():
            ...
            if not left_subject.equals(right_subject):
                raise ValueError(f"subject alignment mismatch for outcome={outcome} family={family_var}")
            ...
            tr_cluster_idx, val_cluster_idx = make_train_val_split(
                n_clusters,
                seed=stable_seed("radiology_refit_fixed_cluster_split", outcome, family_var),
                val_frac=train_cfg["VAL_FRAC"],
            )
            cluster_is_train = np.zeros(n_clusters, dtype=bool)
            cluster_is_val = np.zeros(n_clusters, dtype=bool)
            cluster_is_train[np.asarray(tr_cluster_idx, dtype=int)] = True
            cluster_is_val[np.asarray(val_cluster_idx, dtype=int)] = True
...
def make_delete_a_group_partitions(point_store: dict, n_groups: int):
    partitions = {}
    group_rows = []
    ...
    for store_key, payload in point_store.items():
        ...
        order = np.random.default_rng(
            stable_seed("radiology_delete_a_group_partition", outcome, family_var, n_groups)
    ...
    return partitions, pd.DataFrame(group_rows)
...
def fit_delete_group_refit_scheme_fixed(
    ...
    init_seed = stable_seed("radiology_refit_fixed_init", outcome, family_var, scheme_name)
    ...
    model = fit_weighted_neural_cox(
        ...
        dropout=0.1,
        lr=G_TRAIN_CFG["LR"],
        weight_decay=G_TRAIN_CFG["WEIGHT_DECAY"],
        epochs=G_TRAIN_CFG["EPOCHS"],
        lambda_beta=G_TRAIN_CFG["LAMBDA_BETA"],
        lambda_gamma=G_TRAIN_CFG["LAMBDA_GAMMA"],
    )
    ...
    diagnostics = {
        ...
        "n_rows_positive_weight": int((full_weight > 0).sum()),
    ...
    return tbl, diagnostics, keyword_artifact
...
def build_keyword_metadata_store(artifact_store: dict, vec, svd, ref_map: dict):
    ...
    for outcome, by_family in artifact_store.items():
        for family_var, by_scheme in by_family.items():
            ...
            demo_cols = list(artifact["demo_cols"])
            family_demo_cols = [col for col in demo_cols if split_group_and_level(col)[0] == family_var]
            group_frame = make_demo_group_frame(family_demo_cols, ref_map)
...
def build_keyword_point_outputs(artifact_store: dict, keyword_meta_store: dict):
    ...
    for store_key, meta in keyword_meta_store.items():
        ...
        group_terms = meta["group_frame"]["term"].astype(str).tolist()
        gamma_rows = [list(meta["demo_cols"]).index(term) for term in group_terms]
        interaction_score = interaction_score[gamma_rows]
...
def init_worker(point_store, ref_map, vec, svd, train_cfg, keyword_meta_store):
    global G_POINT_STORE, G_REF, G_VEC, G_SVD, G_TRAIN_CFG, G_KEYWORD_META
    G_POINT_STORE = point_store
    G_REF = ref_map
    G_VEC = vec
    G_SVD = svd
    G_TRAIN_CFG = train_cfg
    G_KEYWORD_META = keyword_meta_store
...
def run_one_delete_group_task(task):
    ...
    payload = G_POINT_STORE[store_key]
    n_clusters = int(payload["n_clusters"])
    cluster_keep = np.ones(n_clusters, dtype=float)
    cluster_keep[deleted_cluster_idx] = 0.0
    row_keep = cluster_keep[np.asarray(payload["subject_codes"], dtype=int)].astype(float)
    ...
    for scheme_name in [LEFT_SCHEME, RIGHT_SCHEME]:
        ...
        diag["resampling_method"] = "delete_a_group_jackknife_fixed_family_overlap"
    ...
    diagnostics_rows.append(
        {
            ...
            "family_var": family_var,
            "delete_group": int(delete_group),
            "resampling_method": "delete_a_group_jackknife_fixed_family_overlap",
    ...
    keyword_payload = None
    if keyword_artifact is not None and store_key in G_KEYWORD_META:
        meta = G_KEYWORD_META[store_key]
        gamma_demo = align_gamma_demo_to_reference(
        ...
        shared_score = np.repeat(shared_score_vec.reshape(1, -1), len(meta["group_frame"]), axis=0)
        interaction_all = gamma_demo @ M.T
        group_terms = meta["group_frame"]["term"].astype(str).tolist()
        gamma_rows = [list(meta["demo_cols"]).index(term) for term in group_terms]
        interaction_score = interaction_all[gamma_rows]
        total_score = interaction_score + shared_score
        contribution_score = interaction_score * token_ref_mean.reshape(1, -1)
...
def main():
    compare_results, main_artifacts, vec, svd, ref_map, train_cfg = load_bundle()
    weighted_point_compare = build_weighted_point_compare_results(
        artifact_store=main_artifacts,
        compare_df=compare_results,
        ref_map=ref_map,
        left_scheme=LEFT_SCHEME,
        right_scheme=RIGHT_SCHEME,
    )
    ...
    partitions, group_table = make_delete_a_group_partitions(point_store=point_store, n_groups=N_GROUPS)
    ...
    with cf.ProcessPoolExecutor(
        ...
        initargs=(point_store, ref_map, vec, svd, train_cfg, keyword_meta_store),
    ) as ex:
        ...
        for fut in tqdm(cf.as_completed(futures), total=len(futures), desc="Delete-a-group jackknife"):
            ...
            if res["compare"] is not None:
                compare_parts.append(res["compare"])
            counts_parts.append(res["counts"])
            diagnostics_parts.append(res["diagnostics"])
            if res["keyword_payload"] is not None:
                keyword_payloads.append(res["keyword_payload"])

    jackknife_compare = pd.concat(compare_parts, ignore_index=True) if compare_parts else pd.DataFrame()
    jackknife_counts = pd.concat(counts_parts, ignore_index=True) if counts_parts else pd.DataFrame()
    jackknife_diagnostics = pd.concat(diagnostics_parts, ignore_index=True) if diagnostics_parts else pd.DataFrame()

    jackknife_summary = jackknife_summary_table(
        replicate_df=jackknife_compare,
        point_df=weighted_point_compare,
        group_cols=["outcome", "group_var", "level", "ref_level", "term"],
        metric_cols=WEIGHTED_OUTPUT_METRIC_COLS,
        rep_col="delete_group",
    )

    keyword_replicate_store = build_keyword_replicate_store(keyword_payloads, keyword_meta_store)
    ...
    save_df_pair(weighted_point_compare, OUT_DIR / "jackknife_point_compare.parquet")
